export default {
    ...
    command_replace: '%%',
    regexes: {
        replace: /^(?:let\s+)?(.+?)\s+cooks?\s+instead\s+of\s+(.+)$/im,
        cook: /^(?:let\s+)?(.+?\s+)?cooks?(\s+.+?)?(\s+tomorrow)?(\s+at\s+(.+?)(\s+tomorrow)?)?$/im,
        join: /^(?:let\s+)?(.+?\s+)?(?:joins?|eats?(\s+at)?)(\s+.+)?$/im,
        part: /^(?:let\s+)?(.+?\s+)?(leaves?|parts?)(\s+.+)?$/im,
        list: /^(?:list|info|about)(\s+with\s+(?:hilight|highlight|hl)s?)?(\s+(.+?))?$/im,
        clear: /^(clear|reset|forget)(\s+.+)?$/im,
